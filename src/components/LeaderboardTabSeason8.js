...
const LeaderboardTabSeason8 = (props) => {
	...
	const { data } = usePointsLeaderboard({ account, season: 8 });
	...
	useEffect(() => {
		...
		if (window.twttr?.widgets && data?.users) {
			...
			window.twttr.widgets.load(
				document.getElementById("season-8-share-on-x-btn")
			);
			...
			{data && (
				<>
						...
						<Col>
							<h2 className="d-flex align-items-center">
								<span className="me-auto">
									{"Big Onchain Spring 2.0 - Season 8"}
								</span>
							</h2>
						</Col>
					</Row>
					<Row>
						<Col>
							<p>
								{"Runs from March 1st, 2026 to May 31st, 2026 (11 PM UTC). Points should update in real-time, but will also be manually confirmed sometime in mid-June before awarding prizes."}
							</p>
							<p>
					...
					<Row className="mb-3">
						<Col>
							...
							<ul style={{ listStyle: "disc" }}>
								...
								<li>
									<b>{"25 points (X Post):"}</b>
									{" Share a bird that you own on X and/or post about the Songbirdz project!"}
									<ul style={{ listStyle: "disc" }}>
										<li>
								...
								<li>
									<b>{"50 points (Farcaster Pfp): "}</b>
									{" Use one of your Songbirdz as your profile picture on Farcaster."}
								</li>
								<li>
									<b>{"25 points (Farcaster Post):"}</b>
									{" Share a bird that you own on Farcaster and/or post about the Songbirdz project!"}
									<ul style={{ listStyle: "disc" }}>
										<li>
											{"You must follow and tag "}
											<a
...
LeaderboardTabSeason8.propTypes = {
	onUserClick: PropTypes.func.isRequired,
};

export default LeaderboardTabSeason8;
